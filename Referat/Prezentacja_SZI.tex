...
\begin{document}
		...
		\begin{frame}{Eksperyment}
			Dla danych uczących i sieci neuronowej o 5 neuronach wejściowych, 13 neuronach ukrytych i 1 neuronie wyjściowym. Algorytm kończył działanie, gdy błąd średniokwadratowy jet mniejszy od $0.01$. Otrzymałem następujące wyniki.
		\end{frame}
		\begin{frame}
			\begin{center}		
		...
		\begin{frame}{Wnioski z eksperymentu}
			Z przeprowadzonych testów można wysnuć następujące wniosek, że wartość funkcji błedu oscyluje cały czas koło tej samej liczby. Świadczy to o tym, że znaleźliśmy się w minimum lokalnym.
			\begin{itemize}
			...
			\item Innym powodem może być za mały zbiór uczący, który nie odzwierciedla za dobrze własności, których pragniemy w uczonej przez nas sieci neurowej. Należy pamiętać, żeby zbiór uczący był zawsze odpowiednio duży!
			\end{itemize}
